...
<div class="theater-popup">
    ...
    <div class="theater-panel" data-panel="preset">
        ...
        <div class="theater-section">
            ...
            <p class="theater-hint">选择一个预设的 System Prompt 来控制小剧场的写作风格。</p>
            <div class="theater-btn-row" style="margin-top:0;margin-bottom:8px;">
                <div id="theater-refresh-presets-btn" class="theater-btn"><i class="fa-solid fa-arrows-rotate"></i><span>刷新预设列表</span></div>
            </div>
            <select id="theater-preset-select" class="theater-select">
                <option value="__default__">默认（内置小剧场助手）</option>
                <option value="__custom__">自定义 System Prompt</option>
            </select>

            <div id="theater-preset-preview" style="margin-top:10px;display:none;">
                <label class="theater-label" style="font-size:.8em;">预设内容预览</label>
                <div id="theater-preset-preview-content" class="theater-preview-box"></div>
            </div>

            <div id="theater-custom-prompt-area" style="display:none; margin-top:10px;">
        ...
        <div class="theater-section">
            ...
            <p class="theater-hint">选择要加载的世界书，条目可单独开关。</p>
            <div class="theater-btn-row" style="margin-top:0;margin-bottom:8px;">
                <div id="theater-refresh-wb-btn" class="theater-btn"><i class="fa-solid fa-arrows-rotate"></i><span>刷新列表</span></div>
            ...
            <select id="theater-wb-select" class="theater-select" style="margin-bottom:10px;">
...
function bindPopupEvents() {
    ...
    $d.off('change.tp').on('change.tp', '#theater-preset-select', function () {
        const v = $(this).val();
        settings.selectedPreset = v;
        if (v === '__custom__') {
            $('#theater-custom-prompt-area').show();
            $('#theater-preset-preview').hide();
        } else if (v === '__default__') {
            $('#theater-custom-prompt-area').hide();
            $('#theater-preset-preview').hide();
        } else {
            $('#theater-custom-prompt-area').hide();
            loadPresetContent(v);
    ...
    $d.off('click.trp').on('click.trp', '#theater-refresh-presets-btn', refreshPresetList);
    ...
    // World book selector
    $d.off('click.tlw').on('click.tlw', '#theater-refresh-wb-btn', refreshWorldBookList);
    $d.off('change.twbs').on('change.twbs', '#theater-wb-select', function () {
        const v = $(this).val();
        if (v) loadWorldBookEntries(v);
    });
...
function refreshPresetList() {
    try {
        const select = $('#theater-preset-select');
        select.find('option:not([value="__default__"]):not([value="__custom__"])').remove();

        // Try to read from ST's preset dropdown (chat completion)
        let found = false;
        $('#settings_preset option').each(function () {
            const name = $(this).text().trim();
            const val = $(this).val();
            if (name && val) {
                select.append(`<option value="preset:${val}">${esc(name)}</option>`);
                found = true;
            }
        });

        // Also try text completion presets
        if (!found) {
            $('#settings_preset_textgenerationwebui option').each(function () {
                const name = $(this).text().trim();
                const val = $(this).val();
                if (name && val) {
                    select.append(`<option value="tcpreset:${val}">${esc(name)}</option>`);
                    found = true;
                }
            });
        }
        ...
        if (found) {
            toastr.success('预设列表已刷新');
        } else {
            toastr.warning('未找到可用预设，可使用自定义模式');
        }
    } catch (e) {
        console.error('[Theater] Refresh presets error:', e);
        toastr.error('刷新失败');
    }
}
...
function loadPresetContent(presetId) {
    try {
        ...
        // Try to read from chat completion settings
        if (presetId.startsWith('preset:')) {
            const oai = ctx.chatCompletionSettings;
            if (oai && oai.prompts) {
                content = oai.prompts
                    .filter(p => p.role === 'system' && p.content)
                    .map(p => p.content)
                    .join('\n\n');
            }
        }
        ...
        // Try text completion
        if (!content && ctx.textCompletionSettings?.system_prompt) {
            content = ctx.textCompletionSettings.system_prompt;
        }

        if (content) {
            settings.loadedPresetContent = content;
            saveSettings();
            $('#theater-preset-preview').show();
            $('#theater-preset-preview-content').text(content.substring(0, 500) + (content.length > 500 ? '…' : ''));
        } else {
            $('#theater-preset-preview').hide();
            toastr.info('无法读取该预设内容，将使用默认设置');
        }
    } catch (e) {
        console.error('[Theater] Load preset content error:', e);
    }
}

// ============================================================
// World book list from ST
// ============================================================
function refreshWorldBookList() {
    try {
        ...
        // Try to read from ST's world info dropdown
        // ST uses various selectors for world info, try multiple
        const selectors = [
            '#world_info option',
            '#world_info_select option',
            '.world_info_select option',
            'select[name="world_info"] option',
        ];
        ...
        for (const sel of selectors) {
            $(sel).each(function () {
                const name = $(this).text().trim();
                const val = $(this).val();
                if (name && val && val !== 'None' && val !== '') {
                    select.append(`<option value="${esc(val)}">${esc(name)}</option>`);
                    found = true;
                }
            });
            if (found) break;
        ...
        // Also try to find world info from character data
        if (!found) {
            try {
                const ctx = SillyTavern.getContext();
                if (ctx.characterId !== undefined && ctx.characters[ctx.characterId]) {
                    ...
                    if (charData?.extensions?.world) {
                        select.append(`<option value="char_world">${esc(charData.extensions.world)}</option>`);
                        found = true;
                    }
                }
            } catch (e) { /* ignore */ }
        ...
        if (found) {
            ...
            toastr.warning('未找到世界书，可手动添加条目');
        }
    } catch (e) {
        console.error('[Theater] Refresh world book error:', e);
        toastr.error('刷新失败');
    }
}

function loadWorldBookEntries(worldId) {
    try {
        let entries = [];

        // Try to get world info data from chat metadata
        const ctx = SillyTavern.getContext();
        if (ctx.chatMetadata?.world_info_data?.entries) {
            entries = Object.values(ctx.chatMetadata.world_info_data.entries)
                .filter(e => e.content)
                .map(e => ({
        ...
        if (entries.length === 0 && ctx.extensionSettings?.worldInfo?.entries) {
            entries = Object.values(ctx.extensionSettings.worldInfo.entries)
                .filter(e => e.content)
                .map(e => ({
                    name: e.comment || e.key?.join(', ') || '未命名',
                    content: e.content,
        ...
        if (entries.length > 0) {
            ...
            toastr.warning('未能读取条目内容，请手动添加');
        }
    } catch (e) {
        console.error('[Theater] Load WB entries error:', e);
        toastr.error('读取失败');
    }
}
...
请根据以上角色设定、人设、世界观、剧情上下文和用户指令，生成一个小剧场。严格遵守渲染规则中的输出格式要求。`;
    ...
    const presetVal = settings.selectedPreset || '__default__';
    if (presetVal === '__custom__' && settings.customSystemPrompt) {
        systemPrompt = settings.customSystemPrompt;
    } else if (presetVal !== '__default__' && settings.loadedPresetContent) {
        systemPrompt = settings.loadedPresetContent;
    }
