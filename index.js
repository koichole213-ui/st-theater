...
额外要求 - 交互模式：
- 小剧场必须包含可交互元素（按钮、选择、切换等）
- 使用JavaScript实现点击事件、状态切换、内容展开/收起等交互功能
- 所有可点击元素必须有:active伪类缩放反馈（transform: scale(0.95)）
- 可以包含：选项分支、隐藏内容揭示、角色回复切换、小游戏、投票等
- 交互必须有意义，不能是无效的伪交互`;
...
async function init() {
    ...
    // Wand menu button
    const wandBtn = $(`
        <div id="theater-wand-btn" class="list-group-item flex-container flexGap5" title="小剧场生成器">
    ...
    wandBtn.on('click', () => {
        // Close the extensions menu dropdown first
        $('#extensionsMenu').parent().find('.dropdown-toggle').dropdown('toggle');
        openTheaterPopup();
    });
...
<div class="theater-popup">
    ...
    <div class="theater-panel" data-panel="preset">
        <div class="theater-section">
            ...
            <div id="theater-preset-current" style="${settings.useCurrentPreset ? '' : 'display:none'}; margin-top:12px;">
                <div class="theater-btn-row" style="margin-bottom:10px;">
                    <div id="theater-load-preset-btn" class="theater-btn">
                        <i class="fa-solid fa-arrows-rotate"></i>
                        <span>加载当前预设内容</span>
                    </div>
                </div>
                <div id="theater-preset-sections"></div>
            </div>
            ...
            <div id="theater-custom-prompt-area" style="${settings.useCurrentPreset ? 'display:none' : ''}; margin-top:12px;">
                <textarea id="theater-custom-prompt" class="theater-textarea" rows="8"
                    placeholder="粘贴你想要的预设 System Prompt…">${esc(settings.customSystemPrompt || '')}</textarea>

                <div id="theater-custom-sections"></div>
                ...
                <div class="theater-btn-row">
                    ...
                    <div id="theater-parse-prompt-btn" class="theater-btn">
    ...
    <div class="theater-panel" data-panel="config">
        ...
        <div class="theater-section">
            ...
            <p class="theater-hint">自定义插件面板的样式。修改后点保存生效。所有样式作用于 .theater-popup 内部。</p>
            <textarea id="theater-custom-css" class="theater-textarea theater-css-editor" rows="6"
                placeholder="例如：&#10;.theater-popup { background: #1a1a2e; }&#10;.theater-tab.active { background: #e94560; }&#10;.theater-btn.primary { background: #e94560; }">${esc(settings.customCSS || '')}</textarea>
            <div class="theater-btn-row">
                <div id="theater-save-css-btn" class="theater-btn primary">
                    <i class="fa-solid fa-palette"></i>
                    <span>保存并应用</span>
                </div>
                <div id="theater-reset-css-btn" class="theater-btn danger">
...
function bindPopupEvents() {
    ...
    // Interactive toggle
    $(document).off('change.theater-interactive').on('change.theater-interactive', '#theater-interactive-toggle', function () {
    ...
    $(document).off('change.theater-render').on('change.theater-render', '#theater-render-select', function () {
        ...
        if (val === '__default__') {
            ...
            const tmpl = settings.renderTemplates[parseInt(val)];
    ...
    // Load current preset
    $(document).off('click.theater-load-preset').on('click.theater-load-preset', '#theater-load-preset-btn', loadCurrentPreset);

    // Preset section toggles
    $(document).off('change.theater-section-toggle').on('change.theater-section-toggle', '.theater-section-check', function () {
        const idx = $(this).data('index');
        const source = $(this).data('source');
        if (source === 'preset') {
            if (!settings.presetSectionStates) settings.presetSectionStates = [];
            settings.presetSectionStates[idx] = $(this).is(':checked');
        }
        saveSettings();
    });
    ...
    // Parse custom prompt into sections
    $(document).off('click.theater-parse').on('click.theater-parse', '#theater-parse-prompt-btn', function () {
        const content = $('#theater-custom-prompt').val().trim();
        if (!content) { toastr.warning('请先输入内容'); return; }
        const sections = parseIntoSections(content);
        settings.presetSections = sections;
        settings.presetSectionStates = sections.map(() => true);
        saveSettings();
        renderSections($('#theater-custom-sections'), sections, settings.presetSectionStates, 'custom');
        toastr.success(`已解析为 ${sections.length} 个条目`);
    });

    // History
    $(document).off('click.theater-save-hist').on('click.theater-save-hist', '#theater-save-history-btn', saveToHistory);
    $(document).off('click.theater-copy').on('click.theater-copy', '#theater-copy-html-btn', copyHtml);
    ...
    // Custom CSS
    $(document).off('click.theater-save-css').on('click.theater-save-css', '#theater-save-css-btn', function () {
    ...
    $(document).off('click.theater-reset-css').on('click.theater-reset-css', '#theater-reset-css-btn', function () {
        settings.customCSS = '';
...
function loadCurrentPreset() {
    try {
        const context = SillyTavern.getContext();
        // Try to get system prompt from chat completion settings
        const oai = context.chatCompletionSettings;
        let systemPrompt = '';

        if (oai && oai.prompts) {
            // Chat completion mode - gather enabled prompt items
            const items = oai.prompts.filter(p => p.role === 'system' && p.content);
            if (items.length > 0) {
                ...
                settings.presetSections = sections;
                settings.presetSectionStates = sections.map((_s, i) =>
                    settings.presetSectionStates?.[i] !== undefined ? settings.presetSectionStates[i] : true
                );
                saveSettings();
                renderSections($('#theater-preset-sections'), sections, settings.presetSectionStates, 'preset');
                toastr.success(`已加载 ${sections.length} 个预设条目`);
                return;
            }
        }

        // Fallback: try to get from other API types
        const tc = context.textCompletionSettings;
        if (tc && tc.system_prompt) {
            systemPrompt = tc.system_prompt;
        }

        if (systemPrompt) {
            const sections = parseIntoSections(systemPrompt);
            settings.presetSections = sections;
            settings.presetSectionStates = sections.map(() => true);
            saveSettings();
            renderSections($('#theater-preset-sections'), sections, settings.presetSectionStates, 'preset');
            toastr.success(`已加载 ${sections.length} 个条目`);
        } else {
            toastr.warning('未能读取当前预设内容，请尝试自定义模式');
        }
    } catch (e) {
        console.error('[Theater] Load preset error:', e);
        toastr.error('读取预设失败');
    }
}

function parseIntoSections(text) {
    // Split by double newlines or XML-like tags
...
function renderSections(container, sections, states, source) {
    if (!sections || sections.length === 0) {
        container.html('<p class="theater-empty">无条目</p>');
        return;
    }

    container.html(sections.map((s, i) => `
        <div class="theater-preset-item">
            <label class="theater-preset-item-label">
                <input type="checkbox" class="theater-section-check" data-index="${i}" data-source="${source}"
                    ${states[i] !== false ? 'checked' : ''}>
                <span class="theater-preset-item-name">${esc(s.name)}</span>
            </label>
            <div class="theater-preset-item-preview">${esc(s.content.substring(0, 120))}${s.content.length > 120 ? '…' : ''}</div>
        </div>
    `).join(''));
}

function getActivePresetContent() {
    if (!settings.presetSections || settings.presetSections.length === 0) return '';
    return settings.presetSections
        .filter((_s, i) => settings.presetSectionStates?.[i] !== false)
        .map(s => s.content)
        .join('\n\n');
}
...
async function generateTheater() {
    ...
    if (renderSelect !== '__default__') {
        ...
        if (settings.renderTemplates[idx]) renderRules = settings.renderTemplates[idx].content;
    }

    // Add interactive mode addon
    if (settings.interactiveMode) {
        renderRules += INTERACTIVE_ADDON;
    }
...
请根据以上角色设定、剧情上下文和用户指令，生成一个小剧场。严格遵守渲染规则中的输出格式要求。`;
    ...
    if (settings.useCurrentPreset) {
        const presetContent = getActivePresetContent();
        if (presetContent) systemPrompt = presetContent;
...
function deleteRenderTemplate() {
    ...
    settings.renderTemplates.forEach((t, i) => {
        select.append(`<option value="${i}">${esc(t.name)}</option>`);
    });
...
function importData() {
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            if (data.customCSS !== undefined) { settings.customCSS = data.customCSS; applyCustomCSS(); }
            saveSettings();
...
function refreshInstructionList() {
    ...
    list.html(t.length === 0
        ...
        : t.map((item, i) => `
            <div class="theater-tag" data-index="${i}">
                <span class="theater-tag-text">${esc(item.name)}</span>
                <span class="theater-tag-delete" data-index="${i}"><i class="fa-solid fa-xmark"></i></span>
            </div>`).join(''));
}
...
function refreshHistoryList() {
    ...
    list.html(h.length === 0
        ...
        : h.map((item, i) => `
            <div class="theater-history-item" data-index="${i}">
                <div class="theater-history-header">
                    <span class="theater-history-title">${esc(item.title || `小剧场 #${i + 1}`)}</span>
