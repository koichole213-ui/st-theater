...
function loadPresetEntries() {
        ...
        if (oai?.prompts && Array.isArray(oai.prompts)) {
            cachedPresetEntries = oai.prompts
                ...
                .map((p, i) => ({
                    ...
                    role: p.role || 'system',
                    content: p.content,
                    enabledInST: p.enabled !== false, // ST's own enabled state
                }));
        }
    ...
    // Init states: first load follows ST's state, subsequent loads keep user's choice
    if (!settings.presetEntryStates) settings.presetEntryStates = {};
    cachedPresetEntries.forEach(e => {
        if (!(e.id in settings.presetEntryStates)) {
            // First time seeing this entry: follow ST's enabled state
...
function syncPresetWithST() {
    // Reset all states to match ST's current preset state
    settings.presetEntryStates = {};
    loadPresetEntries();
    toastr.success('已同步酒馆预设状态');
}
