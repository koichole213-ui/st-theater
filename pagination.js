...
export function listPaginationHTML(kind, state) {
    ...
    const button = (action, title, disabled = false) => `<button type="button" class="theater-btn theater-list-page" data-page-action="${action}" ${disabled ? 'disabled' : ''}>${title}</button>`;
    return `<nav class="theater-inst-pagination" data-list-kind="${kind}" data-page-count="${pageCount}" aria-label="${label}分页">
        <span class="theater-page-summary">第 ${page + 1} / ${pageCount} 页 · 共 ${total} 条</span>
        <div class="theater-page-navigation">${button('first', '首页', page === 0)}${button('prev', '上一页', page === 0)}${button('next', '下一页', page === pageCount - 1)}${button('last', '末页', page === pageCount - 1)}</div>
