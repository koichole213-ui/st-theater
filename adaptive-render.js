...
4 过程：进入→阅读/操作→推进→收束，封面/三幕/结尾页按需。动作有提示、反馈、完成状态。不设置跳过整套体验的直接阅读全文入口；局部展开、正常导航可用。
5 音画：仅操作触发的短交互音效，用Web Audio合成；不生成BGM/循环背景声。首次发声必须由读者明确点击开启声音，可关且停止声源；无声也可完成。动画不拖延阅读，遵循prefers-reduced-motion，离场停循环，避免闪烁与密集粒子。
6 技术：仅本页HTML/CSS/JS、内联SVG、合成声；无外部资源/依赖/网络请求/iframe。入口用原生button和click，DOM就绪绑定后才启用、隐藏后文；目标须存在，装饰层pointer-events:none。先推进再播放音效/动画，异常不得阻断进入；不等待声音许可或动画结束解锁正文。初始化失败保留正常阅读，操作失败可重试；连点不重复推进/正文/音源。
7 手机：弹性窄屏、无横向溢出，长文/短屏均可滚动到底；正文≥16px，行高≥1.6，触控≥44px。支持触摸/键盘；拖动有点击等价操作，无需精确手势。
8 自检：完整顺序、入口实际可达后文、返回/连点、音效报错/无声/减少动画、窄屏。检查不输出，无专用标记。`;
...
默认无声，自然滚动/简单翻页；主线直接可读，不逐段点开、不解谜。物件翻开/局部展开/章节定位融入阅读。重构图、材质、文字节奏；不因轻量降成普通文章卡，不强加结尾仪式。`,
    ...
    Object.freeze({
        ...
        rules: `${SHARED_ADAPTIVE_RULES}\n\n【沉浸互动｜参与情境，动作贯穿体验】
优先用CSS绘画表现本篇物件/场景（渐变、伪元素、形状组合），不拿emoji当插画，不挡正文/操作。亲手操作有前→中→后状态，光影/动效/短音效回应。阶段随正文转折，长段安静阅读，不频繁找按钮。可轻探索/轻解谜；收尾承接情绪，不补感言。`,
    }),
    Object.freeze({
        ...
        rules: `${SHARED_ADAPTIVE_RULES}\n\n【大胆实验｜主动探索，发现有意义的惊喜】
优先用CSS绘画表现本篇物件/空间（渐变、伪元素、形状组合），不拿emoji当插画，不挡正文/操作。空间阅读/物件组合/分层揭示/转场/拖动/轻解谜自选，不凑数。提示→尝试→反馈→发现→推进，状态可辨、失败可重试。谜题仅用当前内容/中性图形，无外部知识/虚构线索。回看、取消拖动、连点稳定，发现后留阅读空间。`,
    }),
]);
