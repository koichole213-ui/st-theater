...
test('三个 HTML 规则保持紧凑预算，不给正文新增长度上限', () => {
    for (const profile of adaptiveRenderProfiles()) {
        assert.ok(estimateTokenCount(profile.rules) <= 1250, profile.name);
...
test('v4.3.0 版本号在代码、清单、样式头和设置页保持一致', () => {
    const source = readFileSync(new URL('../index.js', import.meta.url), 'utf8');
    const styles = readFileSync(new URL('../style.css', import.meta.url), 'utf8');
    const manifest = JSON.parse(readFileSync(new URL('../manifest.json', import.meta.url), 'utf8'));
    assert.match(source, /const VERSION = '4\.3\.0'/);
    assert.equal(manifest.version, '4.3.0');
    assert.match(styles, /^\/\* 千夜浮梦 · 小剧场生成器 v4\.3\.0/);
