...
export function markFailed(metrics, signal = null) {
    if (!metrics || metrics.failedAt) return;
    metrics.failedAt = Date.now();
    metrics.failureSignal = signal ? String(signal) : null;
}
...
export function summarizeMetrics(metrics) {
    ...
    return `开始 ${started}；路径 ${metrics.path}；首字 ${ms(metrics.firstTokenAt)}；主体完成 ${ms(metrics.completedAt)}；fallback ${ms(metrics.fallbackAt)}${metrics.fallbackFrom ? `（来自 ${metrics.fallbackFrom}）` : ''}${metrics.failedAt ? `；失败 ${ms(metrics.failedAt)}${metrics.failureSignal ? `（${metrics.failureSignal}）` : ''}` : ''}`;
}
